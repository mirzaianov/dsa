async function fetchBranchNames(username, repo) {
  ...
  while (nextPage !== null) {
    const { nextPage: nextPageNumber, repos: reposPage } = await fetchPageOfBranches(
      username,
      repo,
      nextPage,
      perPage,
...
async function fetchPageOfBranches(username, repo, page, perPage) {
  const response = await fetch(
    `https://api.github.com/repos/${username}/${repo}/branches?page=${page}&per_page=${perPage}`,
  );
